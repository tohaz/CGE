...
namespace aui {
  ...
  void APopupMenu::CalculateLayout(UINT32& outW, UINT32& outH) {
    ...
    if (finalW < mMinWidth) finalW = mMinWidth;
  ...
  void APopupMenu::Draw() {
    ...
    if (mStyle == AUIWidgetStyle::Simple3D && GetRenderPicture() != None) {
      ...
      XRenderFillRectangle(d, PictOpOver, GetRenderPicture(), &dark, 0, SafeINT32(szy) - mDepth, szx, SafeUINT32(mDepth));
      XRenderFillRectangle(d, PictOpOver, GetRenderPicture(), &dark, SafeINT32(szx) - mDepth, 0, SafeUINT32(mDepth), szy);
    }
    ...
    for (size_t i = 0; i < mItems.size(); ++i) {
      ...
      if (item.isSeparator) {
        ...
        if (mSeparatorHeight > 1) {
          ...
          XDrawLine(d, bb, gc, mDepth + mSeparatorPadding, midY + 1, SafeINT32(szx) - mDepth - mSeparatorPadding, midY + 1);
        }
      } else {
        // Draw Text
  ...
  void APopupMenu::OnMouseMove(XEvent* ev) {
    ...
    if (foundIndex != -1 && foundIndex != mHoveredIndex) {
      ...
      if (item.subItems.empty()) {
        ...
        XTranslateCoordinates(d, Wnd(), RootWindow(d, AUIPtr()->Scr()),
                               SafeINT32(SizeX()) - mDepth, itemTopY,
                               &rootX, &rootY, &child);
  ...
  void APopupMenu::OnButtonPress(XEvent* ev) {
    if (ev->xbutton.window != Wnd()) {
      if (mActiveSubMenu && ev->xbutton.window == mActiveSubMenu->Wnd()) return;
