...
// Clamp to actual row count
  ...
  void ATable::DrawRowHeader(Drawable dest, ATableRangeData1 rowStartData, ATableRangeData1 rowEndData) {
    Display *d = AUIPtr()->Disp();
    GC gc = GCPtr();
    ...
    for (INT64 i = rowStartData.cell; i <= rowEndData.cell; i++) {
      ...
      if(currentH > 0 && y_pos + currentH > mColumnHeaderHeight && y_pos < SizeY()) {
        XSetForeground(d, gc, 0xCCCCCC);
        XFillRectangle(d, dest, gc, 0, (INT32) y_pos, (UINT32) mRowHeaderWidth, (UINT32) currentH);
        XSetForeground(d, gc, 0x000000);
        XDrawRectangle(d, dest, gc, 0, (INT32) y_pos, (UINT32) mRowHeaderWidth, (UINT32) currentH);
        std::string label = mRowH[i].second;
        INT32 len = SafeINT32(label.length());
        INT32 text_w = font_info ? XTextWidth(font_info, label.c_str(), len) : (INT32) label.length() * 6;
        XDrawString(d, dest, gc, SafeINT32(static_cast<INT64>(mRowHeaderWidth) - text_w - 4),
            SafeINT32(static_cast<INT64>(y_pos) + (static_cast<INT64>(currentH) / 2) + 4), label.c_str(), len);
      }
      y_pos += currentH;
      if(y_pos >= SizeY())
        break;
  ...
  INT64 ATable::AddRow() {
    ...
    if(!mRowH.contains(signedKey)) {
      ...
      if(label.length() > lastLen) {
        XFontStruct *font_info = Font();
        INT32 len = SafeINT32(label.length());
        INT32 text_width = font_info ? XTextWidth(font_info, label.c_str(), len) : len * 7;
...
// The data area clipping rectangle
    XRectangle globalClip = { SafeINT16(mRowHeaderWidth), SafeINT16(mColumnHeaderHeight), SafeUINT16(
        (INT64) SizeX() - (INT64) mRowHeaderWidth), SafeUINT16((INT64) SizeY() - (INT64) mColumnHeaderHeight) };
...
// Light blue: usually 0xADD8E6 or similar; using a clear sky blue here
        ...
        XFillRectangle(d, dest, gc, SafeINT16(mRowHeaderWidth), SafeINT16(y_pos),
            SafeUINT16((INT64) SizeX() - (INT64) mRowHeaderWidth), SafeUINT16(currentH));
      }
...
// 1. Set per-cell clip for text
          XRectangle cellClip =
              { SafeINT16(x_pos), SafeINT16(y_pos), SafeUINT16(currentW - 1), SafeUINT16(currentH - 1) };
...
// Grid Lines
          ...
          XDrawLine(d, dest, gc, SafeINT32(x_pos + currentW - 1), SafeINT32(y_pos), SafeINT32(x_pos + currentW - 1),
              SafeINT32(y_pos + currentH - 1));
          XDrawLine(d, dest, gc, SafeINT32(x_pos), SafeINT32(y_pos + currentH - 1), SafeINT32(x_pos + currentW - 1),
              SafeINT32(y_pos + currentH - 1));
// Selected Cell Cursor (The dark blue border)
          if(r == mCursorRow && c == mCursorCol) {
            ...
            XDrawRectangle(d, dest, gc, SafeINT32(x_pos), SafeINT32(y_pos), SafeUINT16(currentW - 1),
  ...
  void ATable::OnButtonPress(XEvent *ev) {
    ...
    if(button == Button1) {
      ...
      if(mRowHeaderResizeEnabled && std::abs(x - SafeINT32(mRowHeaderWidth)) < 5) {
      ...
      if(mRowHeightResizeEnabled && x < SafeINT32(mRowHeaderWidth) && y > SafeINT32(mColumnHeaderHeight)) {
        ATableRangeData1 rowStart = Offset2Row(mVOffset);
...
// 1. Basic boundary check: Ignore clicks on headers or scrollbar areas
    if(x < (INT32) mRowHeaderWidth || y < (INT32) mColumnHeaderHeight || x > (INT32) (SizeX() - AUI_TABLE_SCROLL_THICK)
...
// Optimization: stop if we've passed the click point
  ...
  void ATable::OnMouseMove(XEvent *ev) {
    ...
    if(mResizeMode == AUIResizeMode::AUIColumn) {
      ...
      INT32 textW =
          font_info ? XTextWidth(font_info, label.c_str(), (INT32) label.length()) : (INT32) label.length() * 7;
    ...
    } else if(mRowHeightResizeEnabled && x < SafeINT32(mRowHeaderWidth) && y > SafeINT32(mColumnHeaderHeight)) {
      ATableRangeData1 rowStart = Offset2Row(mVOffset);
      INT64 currY = mColumnHeaderHeight - rowStart.offset;
      for (auto const& [id, data] : mRowH) {
        if(id < rowStart.cell)
  ...
  void ATable::UpdateColumnWidthToFit(INT64 colIdx) {
    ...
    for (auto& [rowIdx, cellPtr] : itCol->second) {
      if(cellPtr) {
        INT32 textW =
            font_info ?
                XTextWidth(font_info, cellPtr->data.c_str(), (INT32) cellPtr->data.length()) :
    ...
    INT32 headerW =
        font_info ?
            XTextWidth(font_info, headerLabel.c_str(), (INT32) headerLabel.length()) : (INT32) headerLabel.length() * 7;
...
// Start drawing at mRowHeaderWidth, adjusted back by the internal cell offset
    ...
    XRectangle clip = { SafeINT16(mRowHeaderWidth), 0, SafeUINT16(SizeXUI32() - mRowHeaderWidth), SafeUINT16(
        mColumnHeaderHeight) };
    ...
    for (INT64 i = colStartData.cell; i <= colEndData.cell; i++) {
      ...
      if(currentW > 0 && x_pos < (INT64) SizeX()) {
        ...
        INT32 text_width =
            font_info ? XTextWidth(font_info, label.c_str(), (INT32) label.length()) : (INT32) label.length() * 6;
        INT32 text_x = (INT32) (x_pos + (currentW - text_width) / 2);
        XDrawString(d, dest, gc, text_x, (INT32) ((mColumnHeaderHeight / 2) + 4), label.c_str(),
            (INT32) label.length());
      }
      x_pos += currentW;
      if(x_pos > (INT64) SizeX())
...
// If scrolled to the very end
  ...
  void ATable::AddColumn() {
    ...
    if(!mColumnW.contains(colIdx)) {
      ...
      XFontStruct *font_info = Font();
      INT32 headerTextW =
          font_info ? XTextWidth(font_info, name.c_str(), (INT32) name.length()) : (INT32) name.length() * 7;
      ...
      for (auto& [rowIdx, rowData] : mRows) {
        ...
        if(mAutoWiden) {
          INT32 cellTextW =
              font_info ?
                  XTextWidth(font_info, cell.data.c_str(), (INT32) cell.data.length()) : (INT32) cell.data.length() * 7;
          INT32 padding = 15;
