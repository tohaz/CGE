...
namespace aui {
  ...
  XRenderColor ScaleAndBlend(uint8_t base, uint8_t target, double t) {
    ...
    uint8_t blended8 = static_cast<uint8_t>(base
        + (static_cast<double>(target) - base) * t);
    // Manual scale to 16-bit to avoid any compiler warnings
    uint16_t val16 =
        static_cast<uint16_t>((static_cast<uint32_t>(blended8) << 8) | blended8);
  ...
  UINT32 HLColor(UINT32 ci) {
    ...
    if(overall > 384) {
      c.rgba.r =
          (c.rgba.r > AUI_HL_SHIFT) ?
              static_cast<uint8_t>(c.rgba.r - AUI_HL_SHIFT) :
              static_cast<uint8_t>(0);
      c.rgba.g =
          (c.rgba.g > AUI_HL_SHIFT) ?
              static_cast<uint8_t>(c.rgba.g - AUI_HL_SHIFT) :
              static_cast<uint8_t>(0);
      c.rgba.b =
          (c.rgba.b > AUI_HL_SHIFT) ?
              static_cast<uint8_t>(c.rgba.b - AUI_HL_SHIFT) :
              static_cast<uint8_t>(0);
    } else {
      c.rgba.r =
          (c.rgba.r < (255 - AUI_HL_SHIFT)) ?
              static_cast<uint8_t>(c.rgba.r + AUI_HL_SHIFT) :
              static_cast<uint8_t>(255);
      c.rgba.g =
          (c.rgba.g < (255 - AUI_HL_SHIFT)) ?
              static_cast<uint8_t>(c.rgba.g + AUI_HL_SHIFT) :
              static_cast<uint8_t>(255);
      c.rgba.b =
          (c.rgba.b < (255 - AUI_HL_SHIFT)) ?
              static_cast<uint8_t>(c.rgba.b + AUI_HL_SHIFT) :
              static_cast<uint8_t>(255);
    }
    return c.value;
  ...
  AUI::AUI() {
    ...
    CreateMainWindow();
    D2(
        "init finished, widget size {}, short size {}, int {}, long {}, float {}, double {}, ptr {}",
        sizeof(AWidget), sizeof(short), sizeof(int), sizeof(long),
  ...
  void AUI::ProcessMessages() {
      ...
      while (!mShouldExit) {
        ...
        D2("VALGRIND TRACE: X-Server sent Window ID = {}", static_cast<UINT64>(event.xany.window));
        D2("VALGRIND TRACE: Main Window ID is = {}", static_cast<UINT64>(mMainWnd->Wnd()));
        if(mShouldExit) {
        ...
        if (activeModal) {
            if (event.type == ButtonPress || event.type == ButtonRelease ||
                ...
                if (!IsChildOfModal) {
                    ...
                    continue; // Skip propagation to underlying widgets
                }
            }
        }
        // 2. STABLE PATH: Process standard window events natively
          ...
          if(targetFound) {
            ...
            int localX = 0;
            int localY = 0;
            XTranslateCoordinates(d, XRootWindow(d, 0), targetWin,
                event.xbutton.x_root, event.xbutton.y_root, &localX, &localY,
                &childWinReturn);
  ...
  void AUI::AddWidget(AWidget *w) {
    ...
    D2("adding widget type {}, reg sz={}, wnd {}", (UINT32)w->Type(),
        mWidg.size(), w->Wnd())
    if(wp != 0) {
      D3("widg has parent")
      wp->AddWidgetChild(w);
  ...
  static inline std::map<UINT32, std::string> CGXEventNames { { 2, "KeyPress" },
      { 3, "KeyRelease" }, { 4, "ButtonPress" }, { 5, "ButtonRelease" }, { 6,
          "MotionNotify" }, { 7, "EnterNotify" }, { 8, "LeaveNotify" }, { 9,
          "FocusIn" }, { 10, "FocusOut" }, { 11, "KeymapNotify" }, { 12,
          "Expose" }, { 13, "GraphicsExpose" }, { 14, "NoExpose" }, { 15,
          "VisibilityNotify" }, { 16, "CreateNotify" }, { 17, "DestroyNotify" },
      { 18, "UnmapNotify" }, { 19, "MapNotify" }, { 20, "MapRequest" }, { 21,
          "ReparentNotify" }, { 22, "ConfigureNotify" }, { 23,
          "ConfigureRequest" }, { 24, "GravityNotify" },
      { 25, "ResizeRequest" }, { 26, "CirculateNotify" }, { 27,
          "CirculateRequest" }, { 28, "PropertyNotify" },
      { 29, "SelectionClear" }, { 30, "SelectionRequest" }, { 31,
          "SelectionNotify" }, { 32, "ColormapNotify" },
      { 33, "ClientMessage" }, { 34, "MappingNotify" }, { 35, "GenericEvent" } };
  ...
  void AUI::PushModal(AWidget* win) {
    if (!win || !mDisplay) [[unlikely]] return;
